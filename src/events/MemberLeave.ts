...
export class MemberLeave {
    constructor(member: GuildMember | PartialGuildMember) {
        ...
        member.createDM()
            .then((dm) => {
                dm.send(`Hi ${member.displayName}! Sorry to hear that you left the server! :( If you would ever like to join back, just let me know! :slight_smile: `);
            })
    }
}
...
const RemoveFromBuddyProject = async (memberId: string) => {
    try {
        ...
        textLog(`(MemberLeave) -> There was an error removing member from Buddy Project DB: ${memberId}`);
    }
}
...
const RemoveFromBirthdays = async (memberId: string) => {
    try {
        ...
        textLog(`(MemberLeave) -> There was an error removing member from Birthday DB: ${memberId}`);
    }
}
