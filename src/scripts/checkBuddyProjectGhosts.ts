...
const checkBuddyProjectGhosts = async (guild: Guild) => {
  ...
  for (let i = 0; i < ghosted.length; i++) {
    ...
    buddyDm.send(
      "Hey there! Unfortunately you didn't click the reaction within the last 7 days and thus I expect you to be a very infrequent Discord user. Hence I unmatched you from your buddy to allow them to get a new match. To enter again, go to the Yes Theory Fam server, to the buddy-project check and click the speech bubble twice and be sure to stick around so you don't miss your buddy again :)"
    );

    const userDm = await user.createDM();
    await userDm.send(
      "Hey there! Your buddy didn't respond to my message either so I unmatched you! I will also start matching you again in just a second so you get another shot!"
    );
...
const resolveUser = (id: string, type: "ghosted" | "buddy", guild: Guild) => {
  ...
  if (!user)
    throw `I couldn't find the user <@${id}> while trying to sort out ghosting! (this would have been the ${type} user)`;
...
const resolveUsers = (
  ...
  guild: Guild
): { buddy: GuildMember; user: GuildMember } => {
...
bot.on("ready", async () => {
  console.log(
    new Date(),
    "Secondary bot instance running to handle ghosts - starting"
  );
