import Discord, { TextChannel, GuildCreateChannelOptions, User, Guild, Client, GuildMember, MessageAttachment } from 'discord.js';
import Tools from '../common/tools';


export default async function AdventureGame(user: User, guild: Guild, bot: Client) {
    ...
    if(attemptedFindChannel) {
        user.createDM().then(channel => {
            channel.send(`You already have an active game session! Go play it here: <#${attemptedFindChannel.id}>`)
        })
        return;
    }
    
    const channel = <TextChannel> await guild.channels.create(channelName,channelOptions)
    ...
    const firstMessage = await channel.send(intro);
     firstMessage.react("🧗").then(reaction => firstMessage.react("🧘"))
        ...
        firstMessage.awaitReactions((reaction: any, user: User) => {
                return !user.bot
...
const userClimb = async (channel:TextChannel) => {
    ...
    emergeMessage.awaitReactions((reaction: any, user: User) => {
        return !user.bot
    }, { max: 1, time: 6000000, errors: ['time'] })
        .then(collected => {
            catsdogs(channel)
...
const catsdogs = async (channel:TextChannel) => {
    ...
    catsDogsMessage.awaitReactions((reaction: any, user: User) => {
        return !user.bot
    }, { max: 1, time: 6000000, errors: ['time'] })
        .then(collected => {
            const reaction = collected.first();
...
const userDied = async (channel:TextChannel) => {
    ...
    deathmessage.react("🐉")
    deathmessage.awaitReactions((reaction: any, user: User) => {
        return !user.bot
    }, { max: 1, time: 6000000, errors: ['time'] })
        .then(collected => {
            const reaction = collected.first();
...
const createChannelOptions = (user:User, bot:Client, guild:Guild): GuildCreateChannelOptions => {
    const categoryId = guild.name == "Yes Theory Fam" ? "684064840800534568" : "694687161085067385";
    const playingRoleId = guild.name == "Yes Theory Fam" ? "694897668547149924" : "694900489015394344";
    return {
...
const lodeWin = async (channel:TextChannel) => {
    ...
    const attachment = new MessageAttachment('https://cdn.discordapp.com/attachments/550445682050334749/694846338482831410/unknown.png');
...
const doraWin = async (channel:TextChannel, user:User) => {
    ...
    const attachment = new MessageAttachment('https://cdn.discordapp.com/attachments/550445682050334749/694846283080400916/unknown.png');
    // Send the attachment in the message channel
    channel.send(attachment);
    channel.send("You can use this profile picture to show your allegiance!")
firstMessage.react("🐉")
