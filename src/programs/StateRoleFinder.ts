...
export default async function StateRoleFinder(pMessage: Discord.Message) {
  ...
  if (!stateRequest) {
    pMessage.reply(
      "Incorrect syntax. Please try again using `!state statename`, with no spaces in the name of the state."
    );
    return;
  }
  const stateRoles = await Tools.resolveFile("stateRoles");
  let found = false;
  let supportRole = pMessage.guild.roles.cache.find(
  ...
  if (pMessage.member.roles.cache.has(supportRole.id)) {
    ...
    if (!found)
      pMessage.reply(
        "I couldn't find that state, please try again using `!state statename`, with no spaces in the name of the state."
      );
  }
}
