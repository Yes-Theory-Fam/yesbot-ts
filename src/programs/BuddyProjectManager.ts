...
export default async function BuddyProjectManager(message: Message, action: string) { 
    switch (action) {
        ...
        case "unmatch":
            const confirm = await message.reply(`Are you sure you would like to remove <@${message.mentions.users.array()[0]}>'s entry?`)
            confirm.react("👍").then(reaction => confirm.react("👎"))
            confirm.awaitReactions((reaction: any, user: User) => {
                    return !user.bot
