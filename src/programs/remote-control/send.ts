...
class SendMessage implements CommandHandler<DiscordEvent.MESSAGE> {
    async handle(message: Message): Promise<void> {
        ...
        if (!channelId || !messageToSend) {
            await Tools.handleUserError(message, "Missing channelId or message to send. Syntax: `!message send {channelId} {message content}`")
            return;
        }
        ...
        if (!channel) {
            await Tools.handleUserError(message, "I could not find that channel! Verify the channelId")
            return;
        }
